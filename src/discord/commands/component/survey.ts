...
> {
    ...
    const selectedOption = survey.questions[questionNumber - 1].options.find(option => option.id === optionID);
    if (!selectedOption) {
        await CreateInteractionResponse(interaction.id, interaction.token, {
            type: InteractionResponseType.ChannelMessageWithSource,
            data: {
            ...
            components: [
                {
                    ...
                    components: survey.questions[questionNumber].options.map(option => ({
