...
import { addBridgeFilter, checkBridgeFilter, getBridgeFilters, getTotalBridgeFilters, removeBridgeFilter } from "@/discord/utils";
        ...
        embeds: [
            {
                title: "Filters",
                color: IsleofDucks.colours.main,
                description: filters.map(filter => `${filter.replace}: ${filter.with}`).join("\n"),
                footer: {
                    text: `Response time: ${Date.now() - timestamp.getTime()}ms`
                },
                timestamp: new Date().toISOString()
