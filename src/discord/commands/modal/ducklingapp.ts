...
> {
    ...
    if (bannedResponse) {
        if (bannedResponse.discord) {
            await BanGuildMember(guildID, bannedResponse.discord, "Automatic ban for being on banlist. Detected through Duckling Application.");
        } else {
            await updateBannedPlayerDiscord(mojang.uuid, member.user.id);
        }
        if (bannedResponse.discord !== member.user.id) {
            await BanGuildMember(guildID, member.user.id, "Automatic ban for being on banlist. Detected through Duckling Application.");
        }
            ...
            embeds: [
                {
                    ...
                    description: "Automatic ban for being on banlist. Detected through Duckling Application.",
                    fields: [
                        {
                            name: "Minecraft UUID",
                            value: `${mojang.uuid}`
        ...
        embeds: [
            {
                ...
                description: `Duckling Application for ${member.nick ?? member.user.username} - ${member.user.id}`,
                color: 0xFB9B00,
            },
            {
                title: mojang.name,
        ...
        components: [
            {
                ...
                components: [
                    {
                        custom_id: `close-ducklingapp-${member.user.id}`,
    ...
    await FollowupMessage(interaction.token, {
        content: `Duckling Application ticket created here: <#${channel.id}>`,
    });
