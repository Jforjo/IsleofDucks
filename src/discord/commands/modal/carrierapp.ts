...
> {
    ...
    for (const profile of profilesRes.profiles) {
        ...
        if (!member) continue;
        for (const [ choice, chose ] of Object.entries(carrierChoices)) {
            if (!chose) continue;
    ...
    const failedRequirements = Object.entries(carrierChoices).filter(([ choice, chose ]) => chose && !carrierRequirementsMet[choice as CarrierAppChoicesType]);
